...
void drawCube() {
    vec3 res = mix(boundingBoxMin, boundingBoxMax, time - floor(time));
    ...
    float excess = res.x + gruix - boundingBoxMax.x > 0 ? res.x + gruix - boundingBoxMax.x : 0.0f;
    float calcWidth = gruix - excess;

    vec3 v1 = vec3(res.x, boundingBoxMin.y, boundingBoxMax.z);
    vec3 v2 = vec3(res.x + calcWidth, boundingBoxMin.y, boundingBoxMax.z);
    vec3 v3 = vec3(res.x, boundingBoxMax.y, boundingBoxMax.z);
    vec3 v4 = vec3(res.x + calcWidth, boundingBoxMax.y, boundingBoxMax.z);
    vec3 v5 = vec3(res.x, boundingBoxMin.y, boundingBoxMin.z);
    vec3 v6 = vec3(res.x + calcWidth, boundingBoxMin.y, boundingBoxMin.z);
    vec3 v7 = vec3(res.x + calcWidth, boundingBoxMax.y, boundingBoxMin.z);
